...
export function operationsGetLiveDebugGrant(
  ...
  Result<
    operations.GetLiveDebugGrantResponse,
    | errors.APIError
    | AlienError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetLiveDebugGrantRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/access-requests/live-debug-grant")();

  const query = encodeFormQuery({
    "debugCloudScope": payload.debugCloudScope,
    "debugNamespace": payload.debugNamespace,
    "debugTool": payload.debugTool,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetLiveDebugGrantResponse$inboundSchema),
