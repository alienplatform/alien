import type { Sandbox, SandboxInstance } from "@alienplatform/sdk"
...
// sandbox it then has to hunt for — and the runtime drops this handler when the runner gives up,
// so cleanup cannot depend on it surviving. The sandbox's own delete flow reaps every sandbox, and
...
app.post("/sandbox-test/:bindingName", async c => {
  ...
  const sandboxId = `e2e-ts-${Date.now()}`
  let instance: SandboxInstance
  try {
    instance = await box.create({ sandboxId })
  } catch (error: unknown) {
    ...
    await box.terminate(sandboxId).catch(() => undefined)
  ...
  try {
    failure = await exercise(box, instance.sandboxId, marker)
  } catch (error: unknown) {
    const alienError = await toExternalOperationError(error, "sandbox-test")
  ...
  // what a reader of this check needs, and a sandbox that also would not close is second to it.
  try {
    await box.terminate(instance.sandboxId)
  } catch (error: unknown) {
...
// sandbox the handler abandoned would not show up in its own answer. Where the backend cannot
...
app.get("/sandbox-sessions/:bindingName", async c => {
  ...
  try {
    ...
    return c.json({ enumerable: true, sessionIds: instances.map(s => s.sandboxId) })
  } catch (error: unknown) {
...
/** Runs a command and moves a file both directions through one sandbox. */
async function exercise(box: Sandbox, sandboxId: string, marker: string): Promise<string | null> {
  let stdout = ""
  let stderr = ""
  let exitCode: number | undefined

  for await (const frame of box.runCommand(sandboxId, "/bin/echo", {
    args: [marker],
    timeoutMs: 30_000,
  })) {
  ...
  await box.writeFiles(sandboxId, { "e2e/input.txt": marker })
  const readBack = await box.readFile(sandboxId, "e2e/input.txt")
  if (readBack.toString("utf8") !== marker) {
    return "readFile returned different bytes than writeFiles sent"
  }

  // Read the same file from inside the sandbox, not only back through the agent. `readFile` is
  ...
  for await (const frame of box.runCommand(sandboxId, "/bin/cat", {
    args: ["e2e/input.txt"],
    timeoutMs: 30_000,
  })) {
  ...
  if (insideExit !== 0) {
    return `the sandbox could not read the file written into it, exit ${insideExit}: ${insideStderr}`
  }
  if (!fromInside.includes(marker)) {
    return `the sandbox read different bytes than writeFiles sent: ${fromInside}`
  }
