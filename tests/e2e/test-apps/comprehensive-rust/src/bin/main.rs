...
fn register_event_handlers(app_state: &AppState) {
    ...
    // Commands test for small request, large response (storage-based response)
    ctx.on_command(
        "cmd-test-large-response",
        |params: serde_json::Value| async move {
            info!(params = ?params, "Received cmd-test-large-response command");
    ...
    // Commands test for medium request (~50KB), small response
    // Tests auto-promote + re-inline path (>20KB KV limit, <150KB transport limit)
    ctx.on_command(
        "cmd-test-medium-request",
        |params: serde_json::Value| async move {
            info!("Received cmd-test-medium-request command");
    ...
    // Commands test for large request, small response
    ctx.on_command(
        "cmd-test-large-request",
        |params: serde_json::Value| async move {
            info!("Received cmd-test-large-request command");
    ...
    // Commands test for large request + large response
    ctx.on_command(
        "cmd-test-large-both",
        |params: serde_json::Value| async move {
            info!("Received cmd-test-large-both command");
