...
export async function checkEnvironmentVariable(agent: Deployment): Promise<void> {
  ...
  await assertResponseOk(response, "environment-variable", "Environment variable check request failed")
  ...
  assertCheck(data.name === varName, "environment-variable", "Unexpected variable name in response", {
