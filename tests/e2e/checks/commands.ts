...
export async function checkCommandSmallPayload(agent: Deployment): Promise<void> {
  ...
  assertCheck(result.testType === "arc-small-payload", "command-small-payload", "Unexpected test type", {
    actualType: result.testType,
  })
  assertCheck(Boolean(result.paramsHash), "command-small-payload", "ARC small payload response missing paramsHash")
}
...
export async function checkCommandLargePayload(agent: Deployment): Promise<void> {
  ...
  assertCheck(result.testType === "arc-large-payload", "command-large-payload", "Unexpected test type", {
    actualType: result.testType,
  })
  assertCheck(
  ...
  assertCheck(result.bulkData.length >= 1000, "command-large-payload", "ARC large payload response too small", {
