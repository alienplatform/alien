...
generated_workflow_version="$(node -e "
  ...
  if (!match) throw new Error('Manager SDK version not found in gen.yaml');
...
generated_lock_version="$(node -e "
  ...
  if (!match) throw new Error('Manager SDK releaseVersion not found in gen.lock');
  process.stdout.write(match[1]);
")"
