...
fn internal(msg: String) -> alien_error::AlienError<ErrorData> {
    alien_error::AlienError::new(ErrorData::InternalError { message: msg })
}
...
async fn load_vault_for_deployment(
    ...
    let deployment = match state.deployment_store.get_deployment(deployment_id).await {
        ...
        Err(e) => return Err(internal(format!("Failed to get deployment: {}", e))),
    };
    ...
    let binding_json = serde_json::to_value(&vault_binding)
        .map_err(|e| internal(format!("Failed to serialize vault binding: {}", e)))?;
    ...
    let provider = BindingsProvider::new(client_config, bindings)
        .map_err(|e| internal(format!("Failed to create bindings provider: {}", e)))?;
...
async fn get_secret(
    ...
    let value = vault
        .get_secret(&key)
        .await
        .map_err(|e| internal(format!("Failed to get secret: {}", e)))?;
