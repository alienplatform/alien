//! `TestAlienOperator` -- helpers for running alien-operator containers in pull mode.
//!
//! In pull mode the alien-operator runs inside a Docker container (or as a Helm
...
/// Find the alien-operator binary.
///
/// Resolution order:
/// 1. `ALIEN_OPERATOR_BINARY` environment variable
/// 2. `target/debug/alien-operator` (or `.exe` on Windows) walking up from CWD
/// 3. `alien-operator` from PATH
fn find_agent_binary() -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    ...
    if let Ok(path) = std::env::var("ALIEN_OPERATOR_BINARY") {
        ...
        return Err(format!(
            "ALIEN_OPERATOR_BINARY set to '{}' but file does not exist",
            path
        )
        .into());
    }

    // 2. Search upward for target/debug/alien-operator
...
/// A running alien-operator container for pull-model E2E tests.
pub struct TestAlienOperator {
    ...
    /// Temp data directory for native operator (cleaned up on drop).
    pub data_dir: Option<tempfile::TempDir>,
    /// Temp file holding the sync token (`--sync-token-file`); deleted on drop.
    /// The operator rejects plaintext token args because argv is visible in `ps`.
    ...
    /// The platform this operator targets.
    pub platform: Platform,
    /// Whether the operator was installed as an OS service via `alien-deploy deploy`.
...
impl TestAlienOperator {
    /// Start an alien-operator Docker container that connects to the given
    /// `TestManager`.
    ///
    /// The container is labelled with [`TEST_AGENT_LABEL`] for easy cleanup.
    /// `image` is the alien-operator Docker image to run (e.g.
    /// `ghcr.io/alienplatform/alien-operator:latest`).
    ///
    /// For cloud platforms (AWS/GCP/Azure), pass the scoped client
    /// configuration the operator should use inside the target environment.
    pub async fn start_container(
        ...
        info!(
            ...
            "starting alien-operator container"
        );
        ...
        let mut args = vec![
            ...
            // The operator needs to reach the manager on the host network
            "--network".to_string(),
            "host".to_string(),
            // Core operator configuration (env var names match clap `env = "..."` annotations)
            "-e".to_string(),
            ...
            "-e".to_string(),
            format!("OPERATOR_ENCRYPTION_KEY={}", encryption_key),
            // Set ALIEN_API_KEY so the operator's preflight checks
            // Deployment prerequisite checks skip themselves.
            // Without this, cloud platform deployments with public ingress
            // are blocked because the operator thinks it's running standalone.
            "-e".to_string(),
        ...
        if !output.status.success() {
            ...
            return Err(format!("Failed to start alien-operator container: {}", stderr).into());
        }

        let container_id = String::from_utf8_lossy(&output.stdout).trim().to_string();
        info!(%container_id, "alien-operator container started");
    ...
    /// Start the alien-operator as a native foreground process for Local platform.
    ///
    /// Finds the alien-operator binary (via `ALIEN_OPERATOR_BINARY` env, or searches
    /// `target/debug/alien-operator`), then spawns it with the manager URL and token.
    pub async fn start_local_process(
        manager: &TestManager,
    ) -> Result<Self, Box<dyn std::error::Error>> {
        info!(
            manager_url = %manager.url,
            "starting alien-operator as native process (local platform)"
        );

        let binary_path = find_agent_binary()?;
        info!(binary = %binary_path.display(), "found alien-operator binary");
        ...
        // (deleted on drop — held on `TestAlienOperator` for child lifetime) and
        // pass `--*-file` paths.
        use std::io::Write;
        ...
        // No credential injection needed — the operator pulls images through
        // the manager's /v2/ registry proxy, which handles upstream auth.

        let child = cmd.spawn().map_err(|e| {
            format!(
                "Failed to spawn alien-operator at {}: {}",
                binary_path.display(),
                e
            )
        })?;

        info!("alien-operator native process started");
    ...
    /// Install the alien-operator via Helm into a Kubernetes cluster.
    ///
    ...
    pub async fn helm_install(
        ...
        info!(
            ...
            "installing alien-operator via helm"
        );
    ...
    pub async fn helm_install_with_values(
        ...
        info!(
            ...
            "installing alien-operator via helm values file"
        );
    ...
    pub async fn stop(self) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(ref container_id) = self.container_id {
            info!(%container_id, "stopping alien-operator container");
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(format!(
                    "Failed to stop alien-operator container {}: {}",
                    container_id, stderr
                )
                .into());
            }

            info!(%container_id, "alien-operator container stopped");
        }
    ...
    pub async fn helm_uninstall(self) -> Result<(), Box<dyn std::error::Error>> {
        let release = self
            .helm_release
            .as_deref()
            .ok_or("Not a Helm-installed operator")?;
        let namespace = self
            .helm_namespace
            .as_deref()
...
impl TestAlienOperator {
    /// Create a `TestAlienOperator` representing an OS service installed by `alien-deploy deploy`.
    ...
    pub async fn cleanup(mut self) {
        if self.installed_as_service {
            info!("uninstalling alien-operator OS service");
            if let Some(ref deploy_bin) = self.deploy_binary {
                ...
                match output {
                    Ok(o) if !o.status.success() => {
                        ...
                        tracing::warn!(
                            "cleanup: alien-deploy operator uninstall failed: {}",
                            stderr
                        );
                    }
                    Err(e) => {
                        tracing::warn!(error = %e, "cleanup: failed to run alien-deploy operator uninstall");
                    }
                    _ => {
                        info!("alien-operator OS service uninstalled");
                    }
                }
            } else {
            ...
            info!("stopping alien-operator native process");
            if let Err(e) = child.kill().await {
                tracing::warn!(error = %e, "cleanup: failed to kill operator native process");
            }
            // data_dir is cleaned up when TempDir is dropped
        } else if self.container_id.is_some() {
            if let Err(e) = self.stop().await {
                tracing::warn!(error = %e, "cleanup: failed to stop operator container");
            }
        } else if self.helm_release.is_some() {
            if let Err(e) = self.helm_uninstall().await {
                tracing::warn!(error = %e, "cleanup: failed to uninstall operator helm release");
            }
        }
    }
}

/// Collect logs from the alien-operator OS service for debugging.
///
/// Checks multiple sources: systemd journal / macOS log, service status,
/// panic.log, and operator.db existence in the data directory.
pub async fn collect_service_logs() -> String {
    let mut logs = String::new();

    // Default data dir used by alien-deploy operator install.
    ...
    //
    // service-manager 0.10 converts the label "dev.alien.operator" into the script
    // name "{organization}-{application}" = "alien-operator". Systemd uses this as
    // the unit name, while launchd uses the full reverse-DNS label.
    #[cfg(target_os = "linux")]
    {
        // Try both the systemd script name and the full label, since the
        // service-manager maps "dev.alien.operator" → "alien-operator.service".
        for unit_name in ["alien-operator", "dev.alien.operator"] {
        ...
        // Check the unit file (service-manager writes "alien-operator.service")
        for filename in ["alien-operator.service", "dev.alien.operator.service"] {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Ok(o) = tokio::process::Command::new("sudo")
            .args(["launchctl", "print", "system/dev.alien.operator"])
            .output()
            .await
        {
            let stdout = String::from_utf8_lossy(&o.stdout);
        ...
        if let Ok(o) = tokio::process::Command::new("cat")
            .arg("/Library/LaunchDaemons/dev.alien.operator.plist")
            .output()
            .await
        {
            let stdout = String::from_utf8_lossy(&o.stdout);
...
// Allow `Arc<TestManager>` usage (the operator only borrows the manager during
    ...
    match client_config {
        ClientConfig::Aws(config) => {
            let alien_core::AwsCredentials::AccessKeys {
                ...
                return Err("operator container tests require AWS access-key credentials".into());
            };
        ...
        ClientConfig::Azure(config) => {
            let alien_core::AzureCredentials::ServicePrincipal {
                ...
                return Err(
                    "operator container tests require Azure service-principal credentials".into(),
                );
            };
