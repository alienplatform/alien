...
/// Ensure cross-account registry access is configured on the management
/// account's container registry so the target account can pull images.
///
/// This simulates the platform's project-setup step. In production, the
/// Alien platform configures these policies when a project is created.
/// In E2E tests we must set them up explicitly before InitialSetup runs,
/// because Lambda/Cloud Run creation validates image access at deploy time.
///
/// Returns `Some(ImagePullCredentials)` for Azure (token-based pull), `None`
/// for AWS/GCP (IAM-based pull).
...
async fn ensure_aws_ecr_cross_account_access(config: &TestConfig) -> anyhow::Result<()> {
    ...
    // the replicated ECR repository in that region.
    if target.region != ecr_region {
        info!(
            target_region = %target.region,
...
/// Set GAR cross-project IAM bindings so the target account's Cloud Run
/// service agent and service accounts can pull images from the management
