...
/// Add an inline IAM policy to the execution role granting ECR pull permissions
/// for the management account's container registry.
///
/// AWS Lambda uses the execution role to pull the container image during
/// CreateFunction / UpdateFunctionCode. For cross-account ECR access, the
/// execution role needs identity-based ECR permissions in addition to the ECR
/// repo's resource-based policy.
pub async fn grant_execution_role_ecr_pull(
    config: &TestConfig,
    resource_prefix: &str,
) -> anyhow::Result<()> {
    use alien_aws_clients::{IamApi, IamClient};
    use alien_core::{AwsClientConfig, AwsCredentials};
    ...
    let role_name = format!("{}-execution-sa", resource_prefix);
    ...
    let iam_client = IamClient::new(reqwest::Client::new(), cred_provider);
    ...
    let policy = serde_json::json!({
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": format!("arn:aws:ecr:*:{}:repository/*", mgmt_account_id)
            }
        ]
    });
    ...
    iam_client
        ...
        .map_err(|e| {
            anyhow::anyhow!(
                "Failed to add ECR pull policy to execution role {}: {}",
                role_name,
                e
            )
        })?;
    ...
    info!(
        role = %role_name,
        mgmt_account = %mgmt_account_id,
        "Granted ECR cross-account pull permissions to execution role"
    );
