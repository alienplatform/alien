...
use alien_core::{Platform, Stack, StackState, VaultOutputs};
use anyhow::Context;
use tracing::info;
...
async fn ensure_local_azure_manager_vault_access(
    ...
    use alien_azure_clients::authorization::AuthorizationApi;
    ...
    use alien_azure_clients::{AzureAuthorizationClient, AzureTokenCache};
    ...
    // The local OSS Azure push harness runs the manager with target service
    // principal credentials when no workload identity token is available. Azure
    // Key Vault uses data-plane RBAC, so ARM owner/contributor permissions are
    // not enough for the manager vault API to set test secrets.
    for (role_name, role_id) in [
        ...
        let full_assignment_id =
            auth_client.build_role_assignment_id(&vault_scope, assignment_id.clone());
...
async fn azure_target_principal_id(target: &crate::config::AzureConfig) -> anyhow::Result<String> {
    if let Some(principal_id) = target.principal_id.as_ref() {
        return Ok(principal_id.clone());
    }

    let output = tokio::process::Command::new("az")
        .args(["ad", "sp", "show", "--id", &target.client_id, "--query", "id", "-o", "tsv"])
        .output()
        .await
        .context(
...
async fn azure_vault_scope_for_deployment(
    ...
    deployment: &TestDeployment,
    vault_name: &str,
) -> anyhow::Result<alien_azure_clients::authorization::Scope> {
...
fn azure_key_vault_scope_from_resource_id(
    resource_id: &str,
) -> anyhow::Result<alien_azure_clients::authorization::Scope> {
    ...
    Ok(alien_azure_clients::authorization::Scope::Resource {
