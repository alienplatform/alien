mod impl_ {
    ...
    use container_registry::{test_support::RunningRegistry, ContainerRegistry};
    ...
    impl LocalArtifactRegistryManager {
        ...
        async fn start_registry_internal(
            ...
            // No auth — the local registry listens on localhost only.
            // Security boundary is OS process isolation. External image access
            // is secured by the manager's registry proxy (deployment tokens).
...
pub use impl_::LocalArtifactRegistryManager;
