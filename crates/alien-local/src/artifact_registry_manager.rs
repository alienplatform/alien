...
impl LocalArtifactRegistryManager {
    ...
    pub fn new_with_shutdown(
        ...
        let background_task = tokio::spawn(async move {
            Self::monitor_and_recover_loop(state_dir_clone, registries_clone, shutdown_rx).await;
        });
        ...
        for entry in entries {
            ...
            if entry.path().is_dir() {
                ...
                if metadata_file.exists() {
                    if let Err(e) =
                        Self::recover_single_registry(&metadata_file, state_dir, registries).await
                    {
                        warn!(
    ...
    /// 1. Verifies registry exists in manager's tracking
    /// 2. Checks RunningRegistry handle is valid
    /// 3. Verifies storage directory exists (persistent state)
    /// 4. Verifies metadata file exists
    ///
    /// # Arguments
    /// * `id` - Resource identifier
    ///
    /// # Returns
    /// Ok(()) if registry exists and is running, error otherwise
    pub async fn check_health(&self, id: &str) -> Result<()> {
        ...
        // Registry exists and is tracked - the RunningRegistry handles its own lifecycle
        Ok(())
    }
    ...
    ///
    /// # Returns
    /// ArtifactRegistryBinding with current registry URL, or error if not running
