...
pub enum ErrorData {
    ...
    /// Trigger service failed to start or deliver events.
    #[error(
        code = "TRIGGER_SERVICE_ERROR",
        message = "Trigger service error for '{trigger_type}' trigger '{trigger_id}': {reason}",
        retryable = "true",
        internal = "false"
    )]
    TriggerServiceError {
        /// Type of trigger (queue, storage, cron)
        trigger_type: String,
        /// Trigger identifier (binding name or cron expression)
        trigger_id: String,
        /// Reason for the failure
        reason: String,
    },

    /// Invalid cron expression in trigger configuration.
    #[error(
        code = "INVALID_CRON_EXPRESSION",
        message = "Invalid cron expression '{expression}': {reason}",
        retryable = "false",
        internal = "false"
    )]
