...
use alien_core::bindings::{binding_env_var_name, KvBinding, VaultBinding};
...
impl LocalBindingsProvider {
    ...
    pub fn new(state_dir: &Path) -> Result<Arc<Self>> {
        ...
        let queue_manager = Arc::new(LocalQueueManager::new(state_dir.clone()));
    ...
    pub fn queue_manager(&self) -> &Arc<LocalQueueManager> {
        &self.queue_manager
    }
...
impl BindingsProviderApi for LocalBindingsProvider {
    ...
    async fn resolve_runtime_only_binding_env(
        ...
        match self
            ...
            .context(BindingsErrorData::BindingConfigInvalid {
                ...
                env_var: binding_env_var_name(binding_name),
    ...
    async fn load_queue(
        ...
        let queue: Arc<dyn Queue> = Arc::new(LocalQueue::new(queue_path).await.context(
