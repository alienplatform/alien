...
async fn test_function_full_lifecycle() {
    ...
    // 4. Start worker
    let url = manager
        .start_worker("test-func", HashMap::new())
        .await
...
async fn test_start_worker_idempotent() {
    ...
    let url1 = manager
        .start_worker("idempotent-func", HashMap::new())
        .await
        .unwrap();
    let url2 = manager
        .start_worker("idempotent-func", HashMap::new())
...
async fn test_start_without_extract_fails() {
    ...
    let result = manager.start_worker("no-image", HashMap::new()).await;
    assert!(result.is_err());
}
...
async fn test_delete_worker_cleanup() {
    ...
    let extracted_path = manager
        ...
        .unwrap();
    manager
        .start_worker("delete-test", HashMap::new())
        .await
        .unwrap();
...
async fn test_get_binding_returns_local_variant() {
    ...
    manager
        ...
        .unwrap();
    manager
        .start_worker("binding-test", HashMap::new())
        .await
        .unwrap();
...
async fn test_health_check() {
    ...
    manager
        ...
        .unwrap();
    manager
        .start_worker("health-test", HashMap::new())
        .await
        .unwrap();
...
async fn test_multiple_functions_concurrent() {
    ...
    for i in 0..3 {
        ...
        manager
            ...
            .unwrap();
        let url = manager
            .start_worker(&func_id, HashMap::new())
            .await
            .unwrap();
...
async fn test_port_allocation_fallback() {
    ...
    let url_before = manager
        .start_worker("fallback-test", HashMap::new())
        .await
        .unwrap();
