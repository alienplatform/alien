...
impl GarArtifactRegistry {
    ...
    pub async fn new(
        ...
        let pull_service_account_email = config
            ...
            .into_value(&binding_name, "pull_service_account_email")
            .context(ErrorData::BindingConfigInvalid {
                binding_name: binding_name.clone(),
                reason: "Failed to extract pull_service_account_email from binding".to_string(),
            })?;

        let push_service_account_email = config
            .push_service_account_email
            .into_value(&binding_name, "push_service_account_email")
            .context(ErrorData::BindingConfigInvalid {
                binding_name: binding_name.clone(),
                reason: "Failed to extract push_service_account_email from binding".to_string(),
            })?;
