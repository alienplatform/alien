...
pub async fn start_job<T: AgentTransport + ?Sized>(
    ...
    // Sent under the run-command name so `send`'s rule applies unchanged: an unanswered start may
    // still have taken the command, and a job that started must not be started again.
    let response = send(
        transport
