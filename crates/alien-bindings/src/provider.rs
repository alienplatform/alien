    ...
    async fn load_storage(&self, binding_name: &str) -> Result<Arc<dyn Storage>> {
        ...
        let result: Arc<dyn Storage> = match binding {
            ...
            StorageBinding::S3(config) => {
                ...
                let sdk_config = crate::aws_sdk::sdk_config_from_alien_config(aws_config).await?;
                let s3_endpoint = aws_config
                    .service_overrides
                    .as_ref()
                    .and_then(|overrides| overrides.endpoints.get("s3"))
                    .map(String::as_str);

                let storage: Arc<dyn Storage> = Arc::new(S3Storage::new(
                    bucket_name,
    ...
    async fn load_build(&self, binding_name: &str) -> Result<Arc<dyn Build>> {
        ...
        match binding {
            ...
            BuildBinding::Codebuild { .. } => {
                ...
                let client =
                    Arc::new(crate::aws_sdk::codebuild_client_from_alien_config(aws_config).await?);

                let build = Arc::new(
                    CodebuildBuild::new(binding_name.to_string(), binding, client).context(
        ...
        let registry: Arc<dyn ArtifactRegistry> = match binding {
            ...
            ArtifactRegistryBinding::Ecr { .. } => {
                ...
                let registry: Arc<dyn ArtifactRegistry> = Arc::new(
                    EcrArtifactRegistry::new(binding_name.to_string(), binding, aws_config.clone())
                        .await
                        .context(ErrorData::BindingConfigInvalid {
                            binding_name: binding_name.to_string(),
    ...
    async fn load_vault(&self, binding_name: &str) -> Result<Arc<dyn Vault>> {
        ...
        let result: Arc<dyn Vault> = match binding {
            ...
            VaultBinding::ParameterStore(config) => {
                ...
                let client =
                    Arc::new(crate::aws_sdk::ssm_client_from_alien_config(aws_config).await?);
    ...
    async fn load_kv(&self, binding_name: &str) -> Result<Arc<dyn Kv>> {
        ...
        let result: Arc<dyn Kv> = match binding {
            ...
            KvBinding::Dynamodb(config) => {
                ...
                let dynamodb_client =
                    Arc::new(crate::aws_sdk::dynamodb_client_from_alien_config(aws_config).await?);
    ...
    async fn load_queue(&self, binding_name: &str) -> Result<Arc<dyn Queue>> {
        ...
        let result: Arc<dyn Queue> = match binding {
            ...
            QueueBinding::Sqs(config) => {
                ...
                let client =
                    Arc::new(crate::aws_sdk::sqs_client_from_alien_config(aws_config).await?);
    ...
    async fn load_worker(&self, binding_name: &str) -> Result<Arc<dyn Worker>> {
        ...
        match binding {
            ...
            WorkerBinding::Lambda(lambda_binding) => {
                ...
                let client =
                    Arc::new(crate::aws_sdk::lambda_client_from_alien_config(aws_config).await?);

                let function_impl = LambdaWorker::new(client, lambda_binding);
    ...
    async fn load_service_account(
        ...
        match binding {
            ...
            ServiceAccountBinding::AwsIam(aws_binding) => {
                ...
                let client =
                    Arc::new(crate::aws_sdk::sts_client_from_alien_config(aws_config).await?);
