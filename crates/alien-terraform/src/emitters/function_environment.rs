...
use alien_core::{
    function_runtime_environment_contract, import::EmitContext, render_runtime_environment_plan,
    validate_runtime_environment_user_map, ErrorData, Function, Platform,
    ...
    R: RuntimeEnvironmentRenderer<Value = Expression>,
{
    ...
    let plan = function_runtime_environment_contract(platform, &function.id, &function.links);
...
pub struct AwsFunctionEnvironmentRenderer<'a, 'ctx> {
    pub ctx: &'a EmitContext<'ctx>,
    pub registry: &'a TfRegistry,
    pub function_id: &'a str,
}

impl RuntimeEnvironmentRenderer for AwsFunctionEnvironmentRenderer<'_, '_> {
    ...
    fn render_runtime_environment_value(
        ...
        match value {
            ...
            RuntimeEnvironmentValue::CurrentFunctionBindingName => {
                Ok(Some(Expression::String(self.function_id.to_string())))
            }
...
pub struct GcpFunctionEnvironmentRenderer<'a, 'ctx> {
    pub ctx: &'a EmitContext<'ctx>,
    pub registry: &'a TfRegistry,
    pub function_id: &'a str,
}

impl RuntimeEnvironmentRenderer for GcpFunctionEnvironmentRenderer<'_, '_> {
    type Value = Expression;
...
pub struct AzureFunctionEnvironmentRenderer<'a, 'ctx> {
    pub ctx: &'a EmitContext<'ctx>,
    pub registry: &'a TfRegistry,
    pub function_id: &'a str,
...
impl RuntimeEnvironmentRenderer for AzureFunctionEnvironmentRenderer<'_, '_> {
    type Value = Expression;
