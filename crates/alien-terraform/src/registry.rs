//! Per-`(ResourceType, Platform)` dispatch for Terraform emitters.
//!
//! `TfRegistry::built_in()` returns a registry pre-populated with every
//! built-in cloud emitter (AWS / GCP / Azure). Plugins layer on additional
//! `(ResourceType, Platform)` entries via `register(...)`. K8s targets
//! (EKS / GKE / AKS) reuse the cloud emitters keyed by `Platform` plus the
//! K8s identity overlay applied by the generator \u2014 they don't need a
//! separate emitter set.

use crate::emitter::TfEmitter;
...
/// Lookup table from `(ResourceType, Platform)` to a `TfEmitter`.
#[derive(Default)]
pub struct TfRegistry {
    emitters: HashMap<Key, Box<dyn TfEmitter>>,
}
