...
fn registered_kubernetes_module_installs_provider_rendered_helm_values() {
    ...
    assert!(helm.contains(
        "(var.helm_install_enabled && var.remote_operator_enabled) || local.remote_operator_identity_record_count == 0"
    ));
    assert!(helm.contains("use terraform destroy for explicit retirement"));
...
fn product_credentials_secret_is_retained_by_identity_records_and_destroyed_after_helm() {
    ...
    assert!(compact.contains(
        "remote_operator_identity_record_count = length(data.kubernetes_resources.remote_operator_identity_records.objects)"
    ));
    assert!(compact.contains(
        "condition = (var.helm_install_enabled && var.remote_operator_enabled) || local.remote_operator_identity_record_count == 0"
    ));
    assert!(compact.contains(
        "field_selector = \"metadata.name=${local.remote_operator_identity_record_name}\""
    ));
