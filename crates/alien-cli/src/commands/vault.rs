...
/// Validate a vault or secret name to prevent path traversal attacks.
...
pub async fn vault_task(args: VaultArgs, port: u16) -> Result<()> {
    ...
    // Vault path matches the running deployment's convention:
    // {state_dir}/{deployment_id}/vault/
    let vault_path = state_dir.join(&deployment_id).join("vault");
