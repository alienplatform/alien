...
pub async fn build_command(args: BuildArgs) -> Result<()> {
    let output = build_task(&args).await?;
    if args.json {
        print_json(&output)?;
    } else {
...
mod tests {
    ...
    fn load_build_output_collects_existing_artifacts() {
        ...
        let args = BuildArgs {
            ...
            targets: None,
            aws_managing_account_id: None,
            cache_url: None,
            json: false,
        };

        let output = load_build_output(&args, output_dir.display().to_string(), 1.25).unwrap();
