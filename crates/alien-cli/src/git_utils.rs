...
/// Get a human-readable description of the current git state
#[allow(dead_code)]
pub fn get_git_description<P: AsRef<Path>>(repo_path: P) -> String {
    match collect_git_metadata(repo_path) {
        Ok(metadata) => {
            let mut parts = Vec::new();

            if let Some(ref inner) = metadata.0 {
                ...
                if let Some(ref sha) = inner.commit_sha {
                    ...
                    parts.push(format!("({})", short_sha));
                }

                if let Some(true) = inner.dirty {
