...
use alien_core::{
    AwsClientConfig, AwsManagementConfig, AzureClientConfig, AzureManagementConfig, ClientConfig,
    GcpClientConfig, GcpManagementConfig, InitialSetupAuthority, KubernetesClientConfig, Platform,
    Resource, ResourceDefinition, ResourceHeartbeat, ResourceOutputs, ResourceRef, ResourceStatus,
    StackState,
};
...
pub struct ResourceControllerContext<'a> {
    ...
    /// Type-indexed provider services used by platform-specific controllers.
...
pub struct ResourceControllerStepResult {
    /// Suggested duration to wait before the next executor step.
    ///
    /// # Delay semantics
    ///
    /// The executor aggregates delays across all resources using `min()`. This means
    /// a single `None` from any resource collapses the delay to zero for the entire step.
    ///
    /// - **`None`** = "proceed immediately." Use for state transitions where the next
    ///   handler should run right away (e.g., `CreateStart` → `CreateWaitForActive`).
    ///
    /// - **`Some(duration)`** = "wait this long." Use for polling (e.g., `Some(3s)` while
    ///   waiting for a resource to become active) and heartbeat checks (e.g., `Some(30s)`
    ///   from a `Ready` handler).
    ///
    /// # Critical invariant: Ready handlers MUST return `Some(duration)`
    ///
    /// A `Ready` handler returning `None` is a bug. It tells the executor "call me again
    /// immediately" which (a) wastes CPU on pointless heartbeat checks and (b) collapses
    /// polling delays from other resources to zero via `min(0, Xs) = 0`, causing them to
    /// burn through their polling budget in seconds instead of waiting between polls.
    ///
    /// Typical values: `Some(5s)` for local, `Some(30s)` for cloud, `Some(60s)` for
    /// infrastructure resources (networks, clusters).
...
/// Resolves persisted concrete controller state without coupling provider crates
/// to the aggregate controller deserializer.
pub trait ControllerStateResolver: Send + Sync {
...
pub fn normalize_controller_state_for_deserialize(
...
///
/// Returns `Ok(Some(controller))` if it handled the type,
/// `Ok(None)` to fall through to built-in deserializers,
