//! Turso database for Operator state
//!
//! Storage model:
...
/// Operator database for persisting state across restarts
pub struct OperatorDb {
    conn: Arc<Mutex<Connection>>,
}

impl OperatorDb {
    /// Create or open the operator database with encryption
    pub async fn new(data_dir: &str, encryption_key: &str) -> Result<Self> {
        if encryption_key.len() != 64 || !encryption_key.chars().all(|c| c.is_ascii_hexdigit()) {
            return Err(alien_error::AlienError::new(
                crate::error::ErrorData::ConfigurationError {
                    message:
                        "OPERATOR_ENCRYPTION_KEY or OPERATOR_ENCRYPTION_KEY must be exactly 64 hex characters (256 bits)"
                            .to_string(),
                },
            ));
        }
        ...
        let db_path = Path::new(data_dir).join("operator.db");
    ...
    async fn ensure_schema_version(conn: &Connection) -> Result<()> {
        let mut rows = conn
            ...
            .into_alien_error()
            .context(ErrorData::DatabaseError {
                message: "Failed to query operator schema_version".to_string(),
            })?;

        let stored_version = match rows.next().await.into_alien_error().context(
            ErrorData::DatabaseError {
                message: "Failed to fetch operator schema_version row".to_string(),
            },
        )? {
            Some(row) => {
                let value: String =
                    row.get(0)
                        .into_alien_error()
                        .context(ErrorData::DatabaseError {
                            message: "Failed to read operator schema_version value".to_string(),
                        })?;
                Some(value.parse::<u32>().map_err(|_| {
                        alien_error::AlienError::new(ErrorData::IncompatibleOperatorState {
                            found_version: 0,
                            min_supported_version: MIN_SUPPORTED_OPERATOR_SCHEMA_VERSION,
                            current_version: CURRENT_OPERATOR_SCHEMA_VERSION,
                            repair: "Operator state has an invalid schema_version; recreate its local state directory."
                                .to_string(),
                        })
                    })?)
            }
        ...
        if stored_version.is_none() {
            ...
            if existing_state_rows > 0 {
                return Err(alien_error::AlienError::new(
                    ErrorData::IncompatibleOperatorState {
                        found_version: 0,
                        min_supported_version: MIN_SUPPORTED_OPERATOR_SCHEMA_VERSION,
                        current_version: CURRENT_OPERATOR_SCHEMA_VERSION,
                        repair:
                            "Operator state is missing schema_version; recreate its local state directory."
                                .to_string(),
                    },
                ));
            }
        ...
        let version = stored_version.unwrap_or(CURRENT_OPERATOR_SCHEMA_VERSION);
        if version < MIN_SUPPORTED_OPERATOR_SCHEMA_VERSION
            || version > CURRENT_OPERATOR_SCHEMA_VERSION
        {
            return Err(alien_error::AlienError::new(
                ErrorData::IncompatibleOperatorState {
                    ...
                    min_supported_version: MIN_SUPPORTED_OPERATOR_SCHEMA_VERSION,
                    current_version: CURRENT_OPERATOR_SCHEMA_VERSION,
                    repair: "Upgrade the operator or recreate its local state directory."
                        .to_string(),
                },
            ));
        }
        ...
        if stored_version.is_none() {
            conn.execute(
                ...
                (CURRENT_OPERATOR_SCHEMA_VERSION.to_string(),),
            )
            .await
            .into_alien_error()
            .context(ErrorData::DatabaseError {
                message: "Failed to initialize operator schema_version".to_string(),
            })?;
        }
    ...
    async fn count_existing_state_rows(conn: &Connection) -> Result<i64> {
        let mut rows = conn
            ...
            .into_alien_error()
            .context(ErrorData::DatabaseError {
                message: "Failed to count existing operator state rows".to_string(),
            })?;
        ...
        let row = rows
            ...
            .into_alien_error()
            .context(ErrorData::DatabaseError {
                message: "Failed to fetch existing operator state count".to_string(),
            })?
            .ok_or_else(|| {
                alien_error::AlienError::new(ErrorData::DatabaseError {
                    message: "Existing operator state count query returned no rows".to_string(),
                })
            })?;
        ...
        row.get(0)
            .into_alien_error()
            .context(ErrorData::DatabaseError {
                message: "Failed to read existing operator state count".to_string(),
            })
    }
