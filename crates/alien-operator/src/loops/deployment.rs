...
///
/// Applies public_urls, stack_settings from operator config,
...
async fn enrich_config(
    ...
    // Pass through public URLs from operator config
    if operator_config.public_urls.is_some() {
        config.public_urls = operator_config.public_urls.clone();
    }
