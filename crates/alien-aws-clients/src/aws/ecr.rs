...
impl EcrApi for EcrClient {
    ...
    async fn get_download_url_for_layer(
        ...
        let body = serde_json::to_string(&request).into_alien_error().context(
            ErrorData::SerializationError {
                message: format!(
                    "Failed to serialize GetDownloadUrlForLayerRequest for repository '{}', digest '{}'",
                    request.repository_name, request.layer_digest
                ),
            },
        )?;

        self.post_json("GetDownloadUrlForLayer", body, &request.repository_name)
            .await
    }
