...
fn generate_handler_action_enum(name: &Ident, state_enum_name: &Ident) -> TokenStream2 {
    quote! {
        ...
        /// # Delay semantics
        ///
        /// `None` means "proceed immediately" — the executor treats it as zero delay. Use it
        /// for state transitions where the next handler should fire right away.
        ///
        /// `Some(duration)` means "wait this long before the next step." Use it for polling
        /// and heartbeat checks.
        ///
        /// **Critical:** Ready/heartbeat handlers MUST return `Some(duration)`, never `None`.
        /// A `None` from a Ready handler collapses ALL resources' polling delays to zero
        /// (via `min(0, Xs) = 0`), causing spin-loops that burn through polling budgets.
        /// See `ResourceControllerStepResult::suggested_delay` for details.
        #[derive(Debug)]
        pub enum #name {
            /// Continue to the next state with an optional delay.
            /// Use `None` for immediate transitions, `Some(duration)` for polling/heartbeat.
            Continue {
                /// The next state to transition to
                state: #state_enum_name,
                /// Delay before the next step. MUST be `Some(...)` when returning to `Ready`.
        ...
        quote! {
            match &self.state {
                ...
                _ => Err(alien_error::AlienError::new(crate::error::ErrorData::ResourceConfigInvalid {
                    ...
                    // TODO: Populate resource_id for error tracking
                    resource_id: None,
                }))
            }
        }
