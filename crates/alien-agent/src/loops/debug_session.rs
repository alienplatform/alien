...
//!
//! The agent exposes a trait + a no-op `Unimplemented` stub. The real
...
//! [`crate::run_agent_with_cancel_and_debug_loop`].
//!
...
//!   own debug backend without touching `alien-agent` internals.
//!
//! When no loop is wired in, the stub is used, the rest of the agent runs
...
pub trait DebugSessionLoop: Send + Sync + 'static {
    async fn run(self: Arc<Self>, state: Arc<AgentState>);
}
