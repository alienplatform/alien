    ...
    fn emit_resources_with_registry(
        ...
        let FunctionCode::Image { image } = &function.code else {
            return Err(AlienError::new(ErrorData::OperationNotSupported {
                ...
                reason: format!(
                    "function '{}' uses source code; CloudFormation templates require a pre-built image",
                    function.id
                ),
            }));
        };
...
fn lambda_fallback_policy(ctx: &EmitContext<'_>, function: &Function) -> Result<CfExpression> {
...
fn schedule_trigger_resources(function: &Function, logical_id: &str) -> Result<Vec<CfResource>> {
    let mut resources = Vec::new();
...
fn queue_trigger_ids(function: &Function, logical_id: &str) -> Vec<CfExpression> {
...
fn schedule_rule_names(function: &Function, logical_id: &str) -> Vec<CfExpression> {
    let mut index = 0usize;
    let mut ids = Vec::new();
    for trigger in &function.triggers {
...
fn storage_permission_statement_ids(function: &Function, logical_id: &str) -> Vec<CfExpression> {
    function
        .triggers
        .iter()
        .filter_map(|trigger| {
