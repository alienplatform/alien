...
use crate::{
    kubernetes::{events::EventApi, metrics::ContainerMetrics, metrics::MetricsApi, pods::PodApi},
...
pub struct KubernetesWorkloadReadInput {
...
pub async fn read_kubernetes_workload(
    pod_client: &Arc<dyn PodApi>,
    event_client: &Arc<dyn EventApi>,
    metrics_client: &Arc<dyn MetricsApi>,
    input: &KubernetesWorkloadReadInput,
    ...
    let data = match input.data_kind {
        KubernetesWorkloadDataKind::Container => ResourceHeartbeatData::Container(
            alien_core::ContainerHeartbeatData::Kubernetes(KubernetesContainerHeartbeatData {
                ...
                name: input.workload_name.clone(),
