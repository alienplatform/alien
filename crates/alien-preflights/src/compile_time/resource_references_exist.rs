...
impl CompileTimeCheck for ResourceReferencesExistCheck {
    ...
    async fn check(&self, stack: &Stack, _platform: Platform) -> Result<CheckResult> {
        ...
        for (resource_id, resource_entry) in stack.resources() {
            ...
            // Skip "compute-cluster" references — those are auto-materialized
            // by ComputeClusterMutation at deployment time. Verifying their
            // existence at compile-time would block daemons whose cluster
            // gets generated by the mutation a moment later.
            for dep_ref in resource_entry.config.get_dependencies() {
                if dep_ref.resource_type().as_ref() == "compute-cluster" {
                    continue;
                }
