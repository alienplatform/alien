...
pub enum FunctionTrigger {
    ...
    /// Function triggered by storage events (object created, deleted, etc.)
    Storage {
    ...
    /// Function triggered on a schedule (cron expression)
    Schedule {
...
impl FunctionTrigger {
    ...
    /// The function will be invoked when matching events occur on the storage resource.
