...
impl GcpFunctionController {
    ...
    async fn creating_push_subscriptions(
        ...
        for trigger in &cfg.triggers {
            if let alien_core::FunctionTrigger::Storage { storage, events } = trigger {
                info!(function=%cfg.id, storage=%storage.id, "Creating storage trigger infrastructure");
    ...
    async fn creating_scheduler_jobs(
        ...
        if schedule_triggers.is_empty() {
            info!(function=%cfg.id, "No schedule triggers found, skipping scheduler job creation");
            return Ok(HandlerAction::Continue {
        ...
        for (index, cron) in &schedule_triggers {
            ...
            let job = SchedulerJob {
                ...
                description: Some(format!(
                    "Schedule trigger for function '{}' (index {})",
                    cfg.id, index
                )),
            ...
            scheduler_client
                ...
                .context(ErrorData::CloudPlatformError {
                    message: format!(
                        "Failed to create Cloud Scheduler job '{}' for function '{}'",
                        job_id, cfg.id
                    ),
                    resource_id: Some(cfg.id.clone()),
                })?;
        ...
        if triggers_changed {
            ...
            for trigger in &current_config.triggers {
                match trigger {
                    ...
                    alien_core::FunctionTrigger::Schedule { .. } => {
    ...
    async fn deleting_scheduler_jobs(
        ...
        info!(function=%function_config.id, jobs=?self.scheduler_job_names, "Deleting Cloud Scheduler jobs");
...
impl GcpFunctionController {
    ...
    /// Gets the service account email for the function's permission profile.
    ...
    async fn create_storage_trigger(
        ...
        let topic_short_name = format!(
            "{}-{}-{}-notif",
            ctx.resource_prefix, function_config.id, storage_ref.id
        );
        let topic_full_name = format!(
            "projects/{}/topics/{}",
            gcp_config.project_id, topic_short_name
        ...
        let created_notification = gcs_client
            ...
            .context(ErrorData::CloudPlatformError {
                message: format!(
                    "Failed to create GCS notification on bucket '{}' for function '{}'",
                    bucket_name, function_config.id
                ),
                resource_id: Some(function_config.id.clone()),
            })?;
        ...
        // Get service account email for OIDC authentication
        let service_account_email = self.get_service_account_email(ctx, function_config)?;
