...
impl AwsFunctionController {
    ...
    async fn creating_event_source_mappings(
        ...
        for trigger in &config.triggers {
            if let alien_core::FunctionTrigger::Storage {
                storage: storage_ref,
                events,
            } = trigger
            {
    ...
    async fn creating_schedule_triggers(
        ...
        for (index, trigger) in config.triggers.iter().enumerate() {
            if let alien_core::FunctionTrigger::Schedule { cron } = trigger {
                info!(function=%config.id, cron=%cron, index=%index, "Creating EventBridge schedule trigger");

                let rule_name = format!("{}-cron-{}", function_name, index);
    ...
    async fn deleting_event_source_mappings(
        ...
        if !self.s3_permission_statement_ids.is_empty() {
            info!(function=%function_config.id, "Cleaning up S3 storage trigger notifications");
    ...
    async fn deleting_schedule_triggers(
        ...
        if !self.eventbridge_rule_names.is_empty() {
            ...
            for rule_name in &self.eventbridge_rule_names.clone() {
                    ...
                    .await
                {
                    ...
                    info!(function=%function_config.id, rule=%rule_name, "EventBridge rule targets removed");
                }
                ...
                if let Err(e) = eventbridge_client.delete_rule(rule_name).await {
                    ...
                    info!(function=%function_config.id, rule=%rule_name, "EventBridge rule deleted");
                }
            }
            self.eventbridge_rule_names.clear();
        }
        ...
        // (Lambda permissions are removed when the function is deleted)
        self.eventbridge_permission_statement_ids.clear();

        // Continue to DeletingFunction state (linear flow)
        Ok(HandlerAction::Continue {
...
mod tests {
    ...
    async fn test_private_function_skips_url_creation() {
        ...
        // API Gateway and permission should NOT be called for private functions
