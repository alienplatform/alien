...
impl RemoteAccessResolver {
    ...
    async fn resolve_azure_impersonation(
        ...
        let access_config: serde_json::Value =
            serde_json::from_str(&outputs.access_configuration).map_err(|e| {
                AlienError::new(ErrorData::RemoteAccessInvalid {
                    message: format!(
                        "Failed to parse Azure access configuration JSON: {}",
                        e
                    ),
        ...
        let uami_client_id = access_config["uamiClientId"]
            .as_str()
            .ok_or_else(|| {
                AlienError::new(ErrorData::RemoteAccessInvalid {
        ...
        let customer_tenant_id = access_config["tenantId"]
            .as_str()
            .ok_or_else(|| {
                AlienError::new(ErrorData::RemoteAccessInvalid {
        ...
        // Primary: OIDC token exchange (if AZURE_FEDERATED_TOKEN_FILE is set)
        if let Some(token_file) = self.env.get("AZURE_FEDERATED_TOKEN_FILE") {
            info!(
                ...
                "Resolving Azure access via OIDC token exchange"
            );

            let oidc_token = std::fs::read_to_string(token_file)
                .map(|s| s.trim().to_string())
                .map_err(|e| {
                    AlienError::new(ErrorData::AuthenticationFailed {
                        message: format!(
                            "Failed to read OIDC token file '{}': {}",
                            token_file, e
                        ),
                        method: Some("oidc_token_file".to_string()),
                    })
                })?;

            let access_token =
                exchange_oidc_token(&token_url, uami_client_id, &oidc_token).await?;
        ...
        Err(AlienError::new(ErrorData::AuthenticationFailed {
            message:
                "No OIDC token file or SP credentials available for Azure cross-tenant access"
                    .to_string(),
...
async fn exchange_oidc_token(
    ...
    let response = client
        ...
        .map_err(|e| {
            AlienError::new(ErrorData::AuthenticationFailed {
                message: format!("OIDC token exchange request failed: {}", e),
    ...
    let token_response: TokenResponse = response.json().await.map_err(|e| {
        AlienError::new(ErrorData::AuthenticationFailed {
            message: format!("Failed to parse OIDC token exchange response: {}", e),
            method: Some("oidc_token_exchange".to_string()),
        })
    })?;
...
async fn exchange_sp_credentials(
    ...
    let response = client
        ...
        .map_err(|e| {
            AlienError::new(ErrorData::AuthenticationFailed {
                message: format!("SP cross-tenant token request failed: {}", e),
    ...
    let token_response: TokenResponse = response.json().await.map_err(|e| {
        AlienError::new(ErrorData::AuthenticationFailed {
            message: format!("Failed to parse SP token response: {}", e),
            method: Some("sp_cross_tenant".to_string()),
        })
    })?;
