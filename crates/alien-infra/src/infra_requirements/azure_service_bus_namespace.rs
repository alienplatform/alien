...
use azure_mgmt_servicebus::package_2024_01::models::{
    sb_namespace_properties, Resource, SbNamespace, SbNamespaceProperties, TrackedResource,
};
use chrono::Utc;
use serde::Serialize;
...
impl AzureServiceBusNamespaceController {
    ...
    async fn create_start(&mut self, ctx: &ResourceControllerContext<'_>) -> Result<HandlerAction> {
        ...
        let namespace_name = self.namespace_name.as_ref().unwrap();
        let result = client
            .namespaces_client()
            .create_or_update(
    ...
    async fn ready(&mut self, ctx: &ResourceControllerContext<'_>) -> Result<HandlerAction> {
        ...
        if let Some(namespace_name) = &self.namespace_name {
            ...
            let result = client
                ...
                .await;
            let namespace = map_azure_core_021_sdk_error(
                "Azure Service Bus",
                result,
                "namespace get",
                "Azure Service Bus namespace",
...
impl AzureServiceBusNamespaceController {
    ...
    /// - The function's managed identity gets data-plane access (send/receive) on the namespace
    /// - The management UAMI gets data-plane send access for commands dispatch
