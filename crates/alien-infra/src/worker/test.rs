...
pub struct TestWorkerController {
    ...
    /// The worker URL, available after URL creation.
...
impl TestWorkerController {
    ...
    async fn create_start(&mut self, ctx: &ResourceControllerContext<'_>) -> Result<HandlerAction> {
        ...
        info!(
            "   Attempting CreateWorker with Memory: {}",
            target_func.memory_mb
        );

        // Create the worker identifier
        self.identifier = Some(format!("test:worker:{}", target_func.id));
    ...
    async fn create_function_polling(
        ...
        let identifier = self.identifier.as_ref().ok_or_else(|| {
            AlienError::new(ErrorData::ResourceControllerConfigError {
                resource_id: target_func.id.clone(),
                message: "Identifier missing in CreateWorkerPolling state".to_string(),
            })
        })?;
        ...
        if self.create_poll_count <= CREATE_POLL_COUNT {
            info!(
                "→ [test-create] Polling ({}/{}) for worker readiness `{}`",
            ...
            info!(
                "→ [test-create] Polling complete ({}/{}), worker `{}` is active (simulated), proceeding to CreateUrl",
                self.create_poll_count, CREATE_POLL_COUNT, identifier
            );
    ...
    async fn create_url(&mut self, ctx: &ResourceControllerContext<'_>) -> Result<HandlerAction> {
        ...
        info!("→ [test-create] CreateWorkerUrl `{}`", identifier);
    ...
    async fn delete_worker_polling(
        &mut self,
        ctx: &ResourceControllerContext<'_>,
    ) -> Result<HandlerAction> {
        let target_func = ctx.desired_resource_config::<Worker>()?;
        let identifier = self.identifier.as_ref().ok_or_else(|| {
            AlienError::new(ErrorData::ResourceControllerConfigError {
                resource_id: target_func.id.clone(),
                message: "Identifier missing in DeleteWorkerPolling state".to_string(),
            })
        })?;
...
/// Helper worker for skip_serializing_if
