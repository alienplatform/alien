...
impl AwsRemoteStackManagementController {
    ...
    async fn delete_start(&mut self, ctx: &ResourceControllerContext<'_>) -> Result<HandlerAction> {
        ...
        info!(
            role_name = %role_name,
            policy_name = %MANAGED_POLICY_NAME,
            "Attempting to delete management policy"
        );
        ...
        {
            Ok(_) => {
                info!(role_name = %role_name, policy_name = %MANAGED_POLICY_NAME, "Management policy deleted successfully");
            }
            Err(e) => {
                // Check if it's a resource not found error (policy doesn't exist)
                if let Some(alien_client_core::ErrorData::RemoteResourceNotFound { .. }) = &e.error
                {
                    warn!(role_name = %role_name, policy_name = %MANAGED_POLICY_NAME, "Management policy already deleted or never existed");
                } else {
                    return Err(e
                        .context(ErrorData::CloudPlatformError {
                            message: "Failed to delete cross-account management IAM role policy"
                                .to_string(),
                            resource_id: Some(
                                ctx.desired_resource_config::<RemoteStackManagement>()?
                                    .id
...
impl AwsRemoteStackManagementController {
    ...
    fn generate_management_policy_document(
        ...
        let policy_document = serde_json::to_string_pretty(&policy).map_err(|e| {
            AlienError::new(ErrorData::InfrastructureError {
                message: format!("Failed to serialize management IAM policy document: {}", e),
                operation: Some("generate_management_policy_document".to_string()),
                resource_id: Some("management".to_string()),
            })
        })?;
