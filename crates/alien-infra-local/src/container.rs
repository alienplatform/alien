...
use crate::container_utils as local_utils;
...
use alien_core::{
    ...
    Platform, ProviderLifecycleState, PublicEndpointOutput, Queue, ResourceHeartbeat,
        ...
        for link in &config.links {
            ...
            if let Some(resource_state) = ctx.state.resources.get(linked_resource_id) {
                ...
                // Check if it's a Kv resource
                    ...
                    if let Some(kv_mgr) = ctx
                        .services
                        .require::<dyn crate::core::LocalServiceProvider>()?
                        .get_local_kv_manager()
                    {
