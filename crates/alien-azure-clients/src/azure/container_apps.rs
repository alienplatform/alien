...
use crate::azure::models::{
    container_apps::{ContainerApp, ContainerAppCollection},
...
impl ContainerAppsApi for AzureContainerAppsClient {
    ...
    async fn list_container_apps_by_resource_group(
        ...
        let resp = self
            .base
            .execute_request(signed, "ListContainerApps", resource_group_name)
            .await?;
        ...
        let body = resp
            ...
            .into_alien_error()
            .context(ErrorData::HttpResponseError {
                message: "Azure ListContainerApps: failed to read response body".to_string(),
                url: url.clone(),
                http_status: 200,
                http_response_text: None,
                http_request_text: None,
