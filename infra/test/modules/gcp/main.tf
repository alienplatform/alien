...
# Management SA can impersonate AR pull/push SAs (cross-account deployment path)
...
# Manager SA can impersonate AR pull/push SAs directly (proxy push/pull path)
resource "google_service_account_iam_member" "manager_impersonates_ar_pull" {
  provider           = google.management
  service_account_id = google_service_account.ar_pull.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.manager.email}"
...
resource "google_service_account_iam_member" "manager_impersonates_ar_push" {
  provider           = google.management
  service_account_id = google_service_account.ar_push.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.manager.email}"
...
# ── E2E: Artifact Registry repo + SAs ────────────────────────────────────────
# These match the alien-infra GcpArtifactRegistryController pattern:
# a dedicated GAR repo with per-registry pull/push service accounts.
# Separate from alien-test-ar-* SAs which are for alien-bindings unit tests.

resource "google_artifact_registry_repository" "e2e" {
  provider      = google.management
  location      = var.management_region
  repository_id = "alien-e2e"
  format        = "DOCKER"

  depends_on = [google_project_service.management_apis]
}

resource "google_service_account" "e2e_ar_pull" {
  provider     = google.management
  account_id   = "alien-e2e-ar-pull"
  display_name = "Alien E2E AR Pull"
}

resource "google_service_account" "e2e_ar_push" {
  provider     = google.management
  account_id   = "alien-e2e-ar-push"
  display_name = "Alien E2E AR Push"
}

resource "google_project_iam_custom_role" "e2e_ar_pull" {
  provider = google.management
  role_id  = "alien_e2e_ar_pull"
  title    = "Alien E2E AR Pull"
  project  = var.management_project_id
  permissions = [
    "artifactregistry.repositories.downloadArtifacts",
    "artifactregistry.files.get",
...
resource "google_project_iam_custom_role" "e2e_ar_push" {
  provider = google.management
  role_id  = "alien_e2e_ar_push"
  title    = "Alien E2E AR Push"
  project  = var.management_project_id
  permissions = [
    "artifactregistry.repositories.downloadArtifacts",
    "artifactregistry.repositories.uploadArtifacts",
...
resource "google_project_iam_member" "e2e_ar_pull" {
  provider = google.management
  project  = var.management_project_id
  role     = google_project_iam_custom_role.e2e_ar_pull.id
  member   = "serviceAccount:${google_service_account.e2e_ar_pull.email}"
}

resource "google_project_iam_member" "e2e_ar_push" {
  provider = google.management
  project  = var.management_project_id
  role     = google_project_iam_custom_role.e2e_ar_push.id
  member   = "serviceAccount:${google_service_account.e2e_ar_push.email}"
}

# Management SA can impersonate E2E AR SAs (cross-account deployment path)
resource "google_service_account_iam_member" "management_impersonates_e2e_ar_pull" {
  provider           = google.management
  service_account_id = google_service_account.e2e_ar_pull.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.management.email}"
}

resource "google_service_account_iam_member" "management_impersonates_e2e_ar_push" {
  provider           = google.management
  service_account_id = google_service_account.e2e_ar_push.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.management.email}"
}

# Manager SA can impersonate E2E AR SAs directly (proxy push/pull path).
# The proxy runs as the manager SA and generates credentials by impersonating
# the AR push/pull SAs. This is a direct hop, not through the management SA.
resource "google_service_account_iam_member" "manager_impersonates_e2e_ar_pull" {
  provider           = google.management
  service_account_id = google_service_account.e2e_ar_pull.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.manager.email}"
...
resource "google_service_account_iam_member" "manager_impersonates_e2e_ar_push" {
  provider           = google.management
  service_account_id = google_service_account.e2e_ar_push.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.manager.email}"
