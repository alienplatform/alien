...
export function waitUntil(promise: Promise<unknown>): void {
  // Lazily initialize the global manager on first use. This handles
  // HTTP-only apps (e.g. Hono) that never call ctx.run() but still
  // need background tasks to be tracked by the runtime.
  if (!globalManager) {
    getChannel()
      .then(channel => {
        if (!globalManager) {
          globalManager = new WaitUntilManager(channel, `app-${process.pid}`)
        }
      ...
      .catch(() => {
        // No gRPC channel available (not running in alien-runtime).
        // Track locally as best-effort.
        promise.catch(error => {
