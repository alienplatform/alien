 ...
 */
export const AzureServiceActivationImportDataSchema = z
  .object({
    providerNamespace: z
      .string()
      .describe("Resource provider namespace (e.g. `Microsoft.App`)."),
    registered: z
      .boolean()
      .describe("True once the provider is reported `Registered`."),
    subscriptionId: z
      .string()
      .describe("Subscription ID the provider was registered on."),
  })
  .describe(
    "Azure ServiceActivation ImportData — registers an Azure resource\nprovider on the customer's subscription.\n\nAdded by the `AzureServiceActivationMutation` preflight when the\nstack contains resources that depend on a specific Azure RP\n(Microsoft.App, Microsoft.Storage, Microsoft.KeyVault, …)."
  );

export type AzureServiceActivationImportData = z.infer<
  typeof AzureServiceActivationImportDataSchema
>;
