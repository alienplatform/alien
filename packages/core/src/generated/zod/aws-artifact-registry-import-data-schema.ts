/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import * as z from "zod";

/**
 * @description AWS ArtifactRegistry ImportData.
 */
export const AwsArtifactRegistryImportDataSchema = z
  .object({
    accountId: z
      .string()
      .describe("AWS account ID that owns the ECR registry."),
    pullRoleArn: z.string().describe("IAM role ARN with pull-only access."),
    pushRoleArn: z.string().describe("IAM role ARN with push and pull access."),
    region: z.string().describe("AWS region for the registry endpoint."),
    registryEndpoint: z.string().describe("Docker registry endpoint."),
    registryId: z
      .string()
      .describe("Registry identifier in `account:region` form."),
    repositoryPrefix: z
      .string()
      .describe("Prefix used for repositories owned by this stack."),
  })
  .describe("AWS ArtifactRegistry ImportData.");

export type AwsArtifactRegistryImportData = z.infer<
  typeof AwsArtifactRegistryImportDataSchema
>;
