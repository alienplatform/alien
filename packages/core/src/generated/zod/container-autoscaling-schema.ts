 ...
 */
export const ContainerAutoscalingSchema = z
  .object({
    desired: z
      .int()
      .min(0)
      .describe("Initial desired replicas at container creation"),
    max: z.int().min(0).describe("Maximum replicas under load"),
    maxHttpP95LatencyMs: z
      .number()
      .describe("Maximum acceptable p95 HTTP latency in milliseconds")
      .nullish(),
    min: z.int().min(0).describe("Minimum replicas (always running)"),
    targetCpuPercent: z
      .number()
      .describe("Target CPU utilization percentage for scaling (default: 70%)")
      .nullish(),
    targetHttpInFlightPerReplica: z
      .int()
      .min(0)
      .describe("Target in-flight HTTP requests per replica")
      .nullish(),
