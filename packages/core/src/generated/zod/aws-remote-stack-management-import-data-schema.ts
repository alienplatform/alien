/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import * as z from "zod";

/**
 * @description AWS RemoteStackManagement ImportData.
 */
export const AwsRemoteStackManagementImportDataSchema = z
  .object({
    managementPermissionsApplied: z
      .boolean()
      .describe(
        "Whether the management inline policy was attached by the generated stack."
      ),
    roleArn: z.string().describe("Cross-account management role ARN."),
    roleName: z.string().describe("Cross-account management role name."),
  })
  .describe("AWS RemoteStackManagement ImportData.");

export type AwsRemoteStackManagementImportData = z.infer<
  typeof AwsRemoteStackManagementImportDataSchema
>;
