...
export const LeaseRequestSchema = z
  .object({
    ...
    leaseSeconds: z.optional(
      z.int().min(0).describe("Lease duration in seconds")
    ),
    maxLeases: z.optional(
      z.int().min(0).describe("Maximum number of leases to acquire")
    ),
    get target() {
      return CommandTargetSchema.describe(
