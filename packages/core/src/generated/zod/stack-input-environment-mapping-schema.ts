/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import * as z from "zod";
import { StackInputEnvironmentVariableTypeSchema } from "./stack-input-environment-variable-type-schema.js";

/**
 * @description How a resolved stack input is injected into runtime environment variables.
 */
export const StackInputEnvironmentMappingSchema = z
  .object({
    name: z.string().describe("Environment variable name."),
    targetResources: z
      .array(z.string())
      .describe(
        "Target resource IDs or patterns. None means every env-capable resource."
      )
      .nullish(),
    get type() {
      return z
        .union([StackInputEnvironmentVariableTypeSchema, z.null()])
        .optional();
    },
  })
  .describe(
    "How a resolved stack input is injected into runtime environment variables."
  );

export type StackInputEnvironmentMapping = z.infer<
  typeof StackInputEnvironmentMappingSchema
>;
