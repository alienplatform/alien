 ...
 */
export function billingGetPlan(
  client: AlienCore,
  request?: operations.GetWorkspacePlanRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetWorkspacePlanResponse,
    | AlienError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetWorkspacePlanRequest$outboundSchema.optional().parse(value),
  ...
  const path = pathToFunc("/v1/billing/plan")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetWorkspacePlanResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
