...
export function syncContext(
  ...
  Result<
    models.SyncAcquireResponseDeployment,
    | errors.APIError
    | AlienError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SyncContextRequest$outboundSchema.optional().parse(value),
  ...
  const body = encodeJSON("body", payload?.SyncContextRequest, {
    explode: true,
  });

  const path = pathToFunc("/v1/sync/context")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.SyncAcquireResponseDeployment$inboundSchema),
