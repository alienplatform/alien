 ...
 */
export function deploymentsSetTargetAgentVersion(
  client: AlienCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SetDeploymentTargetAgentVersionRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.SetTargetAgentVersionRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1/deployments/{id}/target-agent-version")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "setDeploymentTargetAgentVersion",
    oAuth2Scopes: null,
  ...
  >(
    M.json(
      202,
      operations.SetDeploymentTargetAgentVersionResponse$inboundSchema,
    ),
