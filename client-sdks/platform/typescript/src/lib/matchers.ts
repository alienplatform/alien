...
import { AlienDefaultError } from "../models/errors/aliendefaulterror.js"
import { ResponseValidationError } from "../models/errors/responsevalidationerror.js"
import { ERR, OK, type Result } from "../types/fp.js"
import { type StatusCodePredicate, matchResponse, matchStatusCode } from "./http.js"
...
): MatchFunc<T, E | AlienDefaultError | ResponseValidationError> {
  return async function matchFunc(
    response: Response,
    request: Request,
    options?: { resultKey?: string; extraFields?: Record<string, unknown> },
  ): Promise<[result: Result<T, E | AlienDefaultError | ResponseValidationError>, raw: unknown]> {
    let raw: unknown
    ...
    if (!matcher) {
      return [
        {
          ok: false,
          error: new AlienDefaultError("Unexpected Status or Content-Type", {
            response,
            request,
    ...
    if (matcher.enc === "fail") {
      return [
        {
          ok: false,
          error: new AlienDefaultError("API error occurred", {
            request,
            response,
            body,
          }),
