...
> {
  const parsed = safeParse(
    request,
    (value) => models.InitializeRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/initialize")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json([200, 201], models.InitializeResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
