...
import { AlienManagerDefaultError } from "../models/errors/alienmanagerdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AlienManagerDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AlienManagerDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AlienManagerDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AlienManagerDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
